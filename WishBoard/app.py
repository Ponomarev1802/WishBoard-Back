...
from helpers.middlewares import request_user_middleware,json_response
...
async def create_app(loop):
    ...
    middlewares = [session_middleware(RedisStorage(redis_pool)), json_response, request_user_middleware]
    # init application
...
if __name__ == '__main__':
    ...
    logger.debug('Start server {server.sockets[0].getsockname()}')
    try:
        loop.run_forever()
